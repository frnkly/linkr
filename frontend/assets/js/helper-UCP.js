...
var LinkrAPI	= new Class({
	 ...
	 * Error handling
	 */
	setError : function(m, r) {
		this._errMsg[this._errMsg.length]	= m;
		if (this._debug) alert(m);
		return r ? r : false;
	},
	getError : function(c) {
	...
	htmlInput : function(n, v, o)
	{
		...
		return new Element(document.createElement('input'), o);
	},
	 ...
	 */
	htmlSelectCustom : function(li, opt, test)
	{
		// Select list
		if ($type(opt) != 'object') opt = {};
		// 2.3.9: IE9 compatibility
		//var sel	= new Element('select', opt);
		var sel	= new Element(document.createElement('select'), opt);
